...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
    ...
    for (int i = 0; i < num_particles; i++) {
        ...
        /* Find land markers within the sensor range */
        ...
        */
        ...
        for (unsigned int j = 0; j < transformed_os.size(); j++) {
            ...
            double o_id, o_x, o_y, pr_x, pr_y;
            o_id = transformed_os[j].id;
            ...
            // compute the weight
            double gauss_norm = 1 / (2 * M_PI * std_x * std_y);  // normalization term
            double exponent = pow((o_x - pr_x), 2) / (2 * std_x * std_x) + pow((o_y - pr_y), 2) / (2 * std_y * std_y);  // exponent
...
//    uniform_real_distribution<double> uni_real_dist(0.0, 2.0 * max_weight);
//
//    double beta = 0.0;
//
